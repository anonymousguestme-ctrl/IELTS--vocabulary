...
浏览器会把直接打开的 `file://` 页面和 `http://127.0.0.1:8765/` 当作不同存储空间。请在旧页面点击“导出”，生成 `ielts-vocabulary-backup.json`；再打开服务器页面点击“导入”选择该文件。导入采用词条 ID 合并，不会删除当前记录。
